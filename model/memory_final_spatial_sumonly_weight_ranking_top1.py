...
class Memory(nn.Module):
    ...
    def forward(self, query, keys, train=True):
        ...
        if train:
            # losses
            separateness_loss, compactness_loss = self.gather_loss(
                query, keys, train)
            # read
    ...
    def read(self, query, updated_memory):
        ...
        concat_memory = torch.matmul(
            softmax_score_memory.detach(), updated_memory)  # (b X h X w) X d
        updated_query = torch.cat(
            (query_reshape, concat_memory), dim=1)  # (b X h X w) X 2d
